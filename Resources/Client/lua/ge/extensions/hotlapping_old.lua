...
-- Leaderboard data structures (Этап 7.2)
...
-- Functions for leaderboard data management (Этап 7.2)
local function updatePlayerBestTime(playerName, time, vehicle)
...
local function addPlayerRecentTime(playerName, time, vehicle)
...
local function clearLeaderboardData()
...
-- JSON formats for server communication (Этап 7.2)
local function createLeaderboardUpdateMessage(mapName, playerName, time, vehicle, isNewBest)
    return {
...
local function createLeaderboardRequestMessage(mapName)
    return {
...
-- Server response data processing (Этап 7.2)
local function processLeaderboardUpdate(data)
...
-- Helper functions for player info and server communication (Этап 7.1)
local function getLocalPlayerName()
    if isInMP() and MP and type(MP.GetPlayerName) == "function" then
...
local function sendLapTimeToServer(time, vehicle, isNewBest)
    if not isInMP() then
    ...
    local message = createLeaderboardUpdateMessage(mapName, playerName, time, vehicle, isNewBest)
...
local function requestLeaderboardFromServer(mapName)
    if not isInMP() then
        log("Not in multiplayer mode, skipping leaderboard request", "DEBUG")
        return false
    end
    
    local message = createLeaderboardRequestMessage(mapName)
...
local function onPreRender(dt)
    ...
    if im.Begin("Hotlapping##HotlappingMainWindow", nil, flags) then
        ...
        -- Multiplayer status (Этап 7.1)
        local mpMode = getOperationMode()
...
local function onExtensionLoaded()
    ...
    -- Setup lap completed callback for multiplayer integration (Этап 7.1)
    if lapTimer then
        lapTimer.setOnLapCompletedCallback(function(lapRecord)
            ...
            local playerName = getLocalPlayerName()
            if playerName then
                local isNewBest = updatePlayerBestTime(playerName, lapRecord.time, lapRecord.vehicle)
                addPlayerRecentTime(playerName, lapRecord.time, lapRecord.vehicle)
                
                -- Send to server if in multiplayer mode
                if isInMP() then
                    sendLapTimeToServer(lapRecord.time, lapRecord.vehicle, isNewBest)
                end
            end
        end)
    end
...
local function onClientStartMission(levelPath)
    ...
    -- Request leaderboard from server if in multiplayer (Этап 7.1)
    if isInMP() and storageManager then
        local mapName = storageManager.getCurrentMapName()
        if mapName then
            requestLeaderboardFromServer(mapName)
        end
    end
end
